...
export function Toolbar({ editor }: Props) {
    // Using useCallback hook to memoize the setLink function
    const setLink = useCallback(() => {
        if (!editor) return;  // Guard clause to avoid accessing `editor` if it's null
    ...
    // If there's no editor, render nothing
    if (!editor) {
        return null;
    }
